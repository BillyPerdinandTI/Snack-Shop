 ...
 * @version $Id: medias.php 11118 2025-05-07 19:19:31Z Milbo $
 */
...
//		$this->setUniqueName('file_title');
		$this->setConvertInt(array('virtuemart_media_id', 'virtuemart_vendor_id', 'published', 'file_is_downloadable', 'file_is_forSale', 'file_is_product_image', 'shared', 'is_image'));
...
//		$app = JFactory::getApplication();
			...
			if (!empty($unique_id)) {
				foreach ($unique_id as $item) {
					if ($item['virtuemart_media_id'] != $this->virtuemart_media_id) {
						...
						vmdebug('The file already exists?',$unique_id,$q);
						$this->file_title = $name.'_'.$unique_id[0]['virtuemart_media_id'];
					}
				}
			}
		}
		...
		if($this->is_image === null){
			$this->is_image = VmMediaHandler::isImage($this->file_url);
		}
