 ...
 * @version $Id: shopfunctions.php 11210 2025-10-20 17:01:08Z Milbo $
 */
class ShopFunctions {
...
//							quorvia added a hidden config to remove the od from the rendered list :-)
							if(VmConfig::get('catId_in_catname_tree', 1)){
								$categoryTreeName = $categoryName.' - '.$category->virtuemart_category_id;
							} else {
								$categoryTreeName = $categoryName;
							}
							$categoryTree .= $categoryTreeName . '</option>';
						}
					}
				}
			}
	...
	*/
	...
	static function checkSafePath($sPath=0){
		...
		if(VmConfig::$installed==false or vRequest::getWord('view')== 'updatesmigration') {
			return $safePath;
		}
