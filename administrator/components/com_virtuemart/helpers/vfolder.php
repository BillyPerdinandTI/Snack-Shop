...
/**
 * Derived from the Joomla Framework Filesystem Package
 *
...
abstract class vFolder
{
    ...
    public static function copy($src, $dest, $path = '', $force = false, $useStreams = false)
    {
        ...
        if ($path) {
            $src  = vPath::clean($path . '/' . $src);
            $dest = vPath::clean($path . '/' . $dest);
        }
        ...
        if (!is_dir(vPath::clean($src))) {
            throw new FilesystemException('Source folder not found', -1);
        }

        if (is_dir(vPath::clean($dest)) && !$force) {
    ...
    public static function create($path = '', $mode = 0755)
    {
        ...
        if ($obd != null) {
            ...
            if ($inBaseDir == false) {
                ...
                throw new FilesystemException(__METHOD__ . ': vPath not in open_basedir paths');
            }
        }
        ...
        if (!$ret = @mkdir($path, $mode)) {
            ...
            throw new FilesystemException(__METHOD__ . ': Could not create directory.  vPath: ' . $path);
        }
    ...
    public static function delete($path)
    {
        ...
        if (!is_dir($path)) {
            throw new \UnexpectedValueException(
                sprintf(
                    '%1$s: vPath is not a folder. vPath: %2$s',
                    __METHOD__,
                    vPath::removeRoot($path)
                )
            );
        }
        ...
        throw new FilesystemException(sprintf('%1$s: Could not delete folder. vPath: %2$s', __METHOD__, $path));
    }
	...
	/**
	 * Wrapper for the standard file_exists function, with VirtueMart path filtering.
	 *
    ...
    protected static function _items($path, $filter, $recurse, $full, $exclude, $excludeFilterString, $findfiles)
    {
        ...
        while (($file = readdir($handle)) !== false) {
            if (
                ...
                $fullpath = vPath::clean($path . '/' . $file);
    ...
    public static function listFolderTree($path, $filter, $maxLevel = 3, $level = 0, $parent = 0)
    {
        ...
        if ($level < $maxLevel) {
            ...
            foreach ($folders as $name) {
                ...
                $fullName = vPath::clean($path . '/' . $name);
                $dirs[]   = [
                    'id'       => $id,
                    'parent'   => $parent,
                    'name'     => $name,
