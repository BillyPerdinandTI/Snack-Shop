...
class vmJsApi{
	...
	public static function writeJS(){
		...
		if (vRequest::getCmd ( 'format', 'html') != 'html') return;
	...
	static public function loadBECSS(){
		...
		vmJsApi::css('vtip');
		if(JVM_VERSION<4 and VmConfig::isSiteByApp() and VmConfig::get('FEmanageloadIsisCss',true)){
	...
	* Special, use the second param with "admin" to load files always from the admin area
	...
	*/
	public static function setPath($fileName, $directory = '', $version = '', $useMinified = false, $extension = 'js', $absoluteFilePath = false)
	{
		...
		if(!empty($directory) and $directory == 'admin'){
		...
		//to get this in effect, we need to change loading of the BE css for the new template without path, then this can be overriden in the normal template (sie below)
		if(!empty($client)){
			// Default template's file path
			$relativePaths[] = $client . 'templates/' . $template['template'] . '/html/com_virtuemart/assets/' . $folderName . '/' . $baseName;

			// Default template's minified file path
			$miniRelativePaths[] = $client . 'templates/' . $template['template'] . '/html/com_virtuemart/assets/' . $folderName . '/' . $baseName;
		}
...
jQuery(document).ready(function() { // GALT: Start listening for dynamic content update.
	...
	static function JcountryStateList($stateIds, $prefix='', $suffix='_field') {
		...
		$updateTrigger = '';
		if(JVM_VERSION>3){
			$updateTrigger = 'jQuery("#'.$prefix.'virtuemart_country_id'.$suffix.'").vm2front("setOpt",{field_update_trigger : "chosen:updated"});';
		}
		self::addJScript('vm-countryState'.$prefix,'
		Virtuemart.updateStateList'.$prefix.' = function($) {
		...
		jQuery(document).ready( function(){
			console.log("document ready vm-countryState'.$prefix.'")
			Virtuemart.updateStateList'.$prefix.'();
		});
...
";
		self::addJScript('box'.$container.$activator,$box);
		$done = true;
...
Virtuemart.requiredMsg = '" .addslashes (vmText::_ ('COM_VIRTUEMART_MISSING_REQUIRED_JS'))."';";
		...
		vmJsApi::addJScript('vm-validator',$js,true);
		// not overridable vmJsApi::addJScript('/components/com_virtuemart/assets/js/vmvalidator.js');
		vmJsApi::addJScript('vmvalidator', false, false, true);
