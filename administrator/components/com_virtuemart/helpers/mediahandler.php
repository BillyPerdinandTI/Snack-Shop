...
class VmMediaHandler {
	...
	function __construct($id=0){
		...
		$this->setRealImageSize = VmConfig::isSite() and VmConfig::get('setRealImageSize',1);
	}
	...
	static public function createMedia($table,$type='',$file_mimetype=''){
		...
		$table->is_image = $table->is_image!==null ? $table->is_image: self::isImage($table->file_url);
		...
		//vmdebug('Create media',$media,$table->is_image,$attribsImage);
		if(empty($type)){
	 ...
	 */
	static public function isImage($file_url, $file_extension = null){

		if($file_extension === null) $file_extension = strtolower(JFile::getExt($file_url));

		if($file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif' || $file_extension == 'webp'){
			$isImage = TRUE;
			//vmdebug('isImage '.$file_url);
		} else {
			$isImage = FALSE;
	...
	function displayMediaThumb($imageArgs=array(),$lightbox=true,$effect="rel='group'",$return = true,$withDescr = false,$absUrl = false, $width=0,$height=0){
		...
		if ($this->setRealImageSize and (empty($width) or empty($height))) {
			if (!empty($file_url_thumb) and file_exists($file_url_thumb) and !is_dir($this->file_url) ) { // Spiros
				//vmStartTimer('thumbs');
	...
	function getIcon($imageArgs,$lightbox,$return=false,$withDescr=false,$absUrl = false){
		...
		if(!empty($this->file_extension)){
			...
			if(!isset($exists[$this->file_extension])){
				$exists[$this->file_extension] = file_exists(VMPATH_ROOT.'/'.$tC);
			}
	...
	function displayIt($file_url, $file_alt, $imageArgs, $lightbox, $effect ="class='modal'",$withDesc=false,$absUrl = false){
		...
		if ($this->setRealImageSize and (empty($imageArgs['width']) or empty($imageArgs['height'])) and $this->is_image ) { // Spiros
	...
	function processAction($data){
		...
		if( $data['media_action'] == 'upload' ){
			...
			$this->file_url='';
			$this->file_url_thumb='';
			$file_name = vmUploader::uploadFile($this->file_url_folder,$this,false); //$this->uploadFile($this->file_url_folder);
			...
			$file_name = vmUploader::uploadFile($this->file_url_folder,$this,true); //$this->uploadFile($this->file_url_folder,true);
			if ($file_name===false) return false;
			$this->file_name = $file_name;
			$this->file_url = $this->file_url_folder.$this->file_name;
			...
			$file_name = vmUploader::uploadFile($this->file_url_folder_thumb,$this,true); //$this->uploadFile($this->file_url_folder_thumb,true);
			if ($file_name===false) return false;
			$this->file_name = $file_name;
