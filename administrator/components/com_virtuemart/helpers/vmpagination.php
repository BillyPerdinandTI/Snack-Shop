...
class VmPagination extends vObject {
	...
	function getLimitBox ($sequence = 0) {
		...
		if(!VmConfig::isSite()) {
			...
			$link = vmURI::getCurrentUrlBy('get',false,true, array('limit','limitstart'));
			...
			if(count( $sequenceArray)<1) {
				...
				$sequenceArray = array($perRow*5,$perRow*10,$perRow*20,$perRow*50);
			}

			foreach( $sequenceArray as $items ) {
				$limitStart = floor ((float)$this->limitstart / (float)$items) * $items;
				$limits[$items] = JHtml::_( 'select.option', JRoute::_( $link.'&limitstart='.$limitStart.'&limit='.$items, false ), $items );
			}
			...
			$limitStart = floor ((float)$this->limitstart / (float)$this->limit) * $this->limit;
			//$selected = JRoute::_( $link.'&limitstart='.$limitStart.'&limit='.$selected, false );
			//vmdebug('getLimitBox',$this->limitstart,$this->limit,$selected,$limits);
			$js = 'onchange="window.top.location.href=this.options[this.selectedIndex].value"';

			$id = VmHtml::ensureUniqueId('limit');

			//vmdebug('getLimitBox add $link',$link,$limits);
			$html = '<select id="'.$id.'" name="limit" class="inputbox" size="1" '.$js.' >';
			$selT = '';
			foreach($limits as $key => $opt) {
				if($selected == $opt->text) $selT = 'selected="selected"';
				$html .= '<option value="'.$opt->value.'" '.$selT.' >'.$opt->text.'</option>';
				$selT = '';
			}
			$html .= '</select>';
			//this does not work correctly with SEF, because the selected is sefed and is not always in the list
	...
	public function getPagesLinks()
	{
		...
		$this->_additionalUrlParams['option'] = vRequest::getCmd('option','com_virtuemart');
		$this->_additionalUrlParams['view'] = vRequest::getCmd('view','category');

		//add only if not false
		if($id = vRequest::getInt('virtuemart_manufacturer_id',false)){
			$this->_additionalUrlParams['virtuemart_manufacturer_id'] = (int)$id;
		}

		if($id = vRequest::getInt('virtuemart_manufacturercategories_id',false)){
			$this->_additionalUrlParams['virtuemart_manufacturercategories_id'] = (int)$id;
		}

		if($str = vRequest::getCmd('keyword',false) and !empty($str) ){
			$this->_additionalUrlParams['keyword'] = vRequest::vmSpecialChars(strip_tags($str));
		}

		if($str = vRequest::getCmd('orderby',false) and !empty($str) ){
			$this->_additionalUrlParams['orderby'] = vRequest::vmSpecialChars(strip_tags($str));
		}

		if($str = vRequest::getCmd('dir',false) and !empty($str) ){
			$this->_additionalUrlParams['dir'] = vRequest::vmSpecialChars(strip_tags($str));
		}

		if($str = vRequest::getVar('customfields',false)){

			if(is_array($str)){
				foreach($str as $i => $field){
					$str[(int)$i] = strip_tags(vRequest::vmSpecialChars_decode($field));
				}
			}

			$this->_additionalUrlParams['customfields'] = $str;

		}

		if($catId = vRequest::getInt('virtuemart_category_id',false)){
			$this->_additionalUrlParams['virtuemart_category_id'] = (int)$catId;
		}
		...
		//if($itemId) $this->_additionalUrlParams['Itemid'] = $itemId;
...
//vmdebug('getPagesLinks',$chromePath);
	...
	protected function _buildDataObject()
	{
		...
		if ($this->get('pages.current') > 1)
		{
			...
			$data->start->base = '0';
			$data->start->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=0&limit=' . $this->limit);
			$data->previous->base = $page;
			$data->previous->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $page. '&limit=' . $this->limit);
		}
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
			...
			$data->next->base = $next;
			$data->next->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $next. '&limit=' . $this->limit);
			$data->end->base = $end;
			$data->end->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $end. '&limit=' . $this->limit);
		}
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i++)
		{
			...
			if ($i != $this->get('pages.current') || $this->_viewall)
			{
				...
				$data->pages[$i]->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $offset. '&limit=' . $this->limit);
				//vmdebug('VmPagination _buildDataObject',$params . $nextDelim . $this->prefix . 'limitstart=' . $offset. '&limit=' . $this->limit,$data->pages[$i]->link);
			}
		}
		return $data;
	}
