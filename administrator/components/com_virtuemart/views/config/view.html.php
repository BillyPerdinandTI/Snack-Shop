 ...
 * @version $Id: view.html.php 11200 2025-09-29 17:40:53Z Milbo $
 */
...
WHERE published="1"';
	...
	static public function rowShopFrontSet($params, $label, $name, $name2, $name3 = 0, $default = 1, $attrs='class="inputbox"'){
		...
		$h .= '<td>';
		if($name3 !== 0) {
			$h .= JHtml::_ ('select.genericlist', self::$options, $name3, '', 'value', 'text', $params->get($name3, 1));

			$groups = array('featured', 'discontinued', 'topten', 'latest');
			if(in_array($name, $groups)){

				$justinCat = $name.'_only_cat';
				//$h .= '<td>'.VmHTML::checkbox($justinCat, $params->get($justinCat, 1)).'</td>';
				$h .= '<td><span  uk-tooltip="' . htmlentities(vmText::_('COM_VM_ADMIN_CFG_ONLY_PRODUCTS_ACTIVE_CAT_TIP')) . '">';
				$h .= JHtml::_ ('select.genericlist', self::$options, $justinCat, '', 'value', 'text', $params->get($justinCat, 1));
				//$h .= VmHTML::checkbox($justinCat, $params->get($justinCat, 0));
				$h .= '</span></td>';
			}
