 ...
 * @copyright Copyright (C) 2011- 2025 by the VirtueMart team - All rights reserved.
...
class com_virtuemartInstallerScript {
	...
	public function loadVm($fresh = true) {
		...
		if($this->path){
			//We cannot load VmConfig here, because it would load the old files.
			if(!class_exists('vmDefines')) {
				if (file_exists($this->path . '/administrator/components/com_virtuemart/helpers/vmdefines.php')) {
					require_once $this->path . '/administrator/components/com_virtuemart/helpers/vmdefines.php';
				} else {
					JFactory::getApplication()->enqueueMessage('Cannot find vmDefines.php in ' . $this->path . '/administrator/components/com_virtuemart/helpers/vmdefines.php', 'error');
					return false;
				}
			} else {
				JFactory::getApplication()->enqueueMessage('vmDefines.php already loaded. If you get an error, or to ensure a proper installation, just install VirtueMart again. This messages appears, because a 3rd party plugin loaded already vmDefines before the installation process started. The 3rd party should add a check if we are in the installation context.', 'warning');
			}

			if(class_exists('vmDefines') and method_exists('vmDefines','defines')){
				vmDefines::defines('administrator',$this->path);
			} else {
				JFactory::getApplication()->enqueueMessage('vmDefines.php not found, but should still be copied. Simply try the installation again.');
				return false;
				if(JVM_VERSION<6){
		...
		if(!class_exists('vmEcho')){
			JFactory::getApplication()->enqueueMessage('Cannot find vmecho.php in ' . $this->path . ' /administrator/components/com_virtuemart/helpers/vmecho.php', 'error');
			return false;
		}
		if(isset(VmConfig::$defined)) VmConfig::$defined = true;

		//Check our new parameter if the config got already loaded
		if(isset(VmConfig::$confLoaded)){
			$loaded = VmConfig::$confLoaded;
		} else {	//Use this workaround to determine if the config was loaded already.
			$loaded = VmConfig::get('dangeroustools','NotSet');
			if($loaded == 'NotSet'){
				$loaded = false;
			} else {
				$loaded = true;
	...
	public function preflight ($type, $parent=null) {
		...
		if(class_exists('JFactory')){
			...
			Joomla\CMS\Factory::getApplication()->enqueueMessage('VirtueMart does not support Joomla versions higher than 5.x yet. Please use Joomla 5 or wait until VirtueMart 5 is released.','error');
			return false;
		}
	...
	public function update ($loadVm = true) {
		...
		$this->deleteTableColumn('#__virtuemart_order_calc_rules', array('locked_on','locked_by'));
...
/*		$extension = 'com_virtuemart';
	 ...
	 * @param array $fields
	 */
	private function deleteTableColumn( $tablename, $fields){
		...
		foreach($fields as $field){
			...
			if(in_array($field,$columns)){
				...
				try {
					$this->_db->execute();
					vmInfo('Deleted old column '.$field.' from '.$tablename);
				} catch (Exception $e) {
					vmInfo('Error: Install deleteTableColumn '.$e->getMessage() );
				}
			}
		}
	...
	*/
	...
	public function displayFinished($update){
		...
		ob_start();
		$instPath = $this->path;
		include ($this->path.'/administrator/components/com_virtuemart/views/updatesmigration/tmpl/insfinished.php');
		$html = ob_get_contents();
		ob_end_clean();
		echo $html;
		if(class_exists('vRequest')) vRequest::setVar('core_html',$html);
		$_REQUEST['core_html'] = $html;//*/
