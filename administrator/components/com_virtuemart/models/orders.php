 ...
 * @version $Id: orders.php 11111 2025-04-07 08:44:40Z Milbo $
 */
...
//quorvia added  ST data searches and virtuemart_order_id and order total
	...
	public function updateSingleItem($virtuemart_order_item_id, &$orderdata, $orderUpdate = false, &$itemTaxes = array(), $orderUserId=0) {
		...
		$today = $date->toSQL();
		//$vatTaxes = array();
		vmdebug('updateSingleItem $virtuemart_order_item_id',$virtuemart_order_item_id,$data['virtuemart_order_item_id']);
		if ( $orderUpdate /* */) {
			$data['order_item_sku'] = trim($data['order_item_sku']);
			$db = JFactory::getDBO();
			if(empty($data['virtuemart_product_id']) and !empty($data['order_item_sku'])){
...
vmdebug('This is empty? $data["order_item_name"]',$data['order_item_name'],(int)empty($data['order_item_name']));
			if(!empty($data['virtuemart_product_id']) and empty($data['order_item_name'])){
				//vmEcho::$echoDebug = 1;

				//This should prevent that a product is sold to a shopper if not allowed and the price is taken from the right shoppergroup
				...
				vmdebug('Loarded product ',$data['virtuemart_product_id']);
			...
			if(!empty($data['calculate_product_tax']) and !empty($data['virtuemart_order_item_id']) ) {
...
/*			if(!empty($bill_rules))
			...
			*/
				...
				//foreach($order_rules as $calc_kind => $calcs) {
					...
					foreach($order_rules as $calc) {
						...
						if ($calc->calc_kind == 'DBTaxRulesBill' || $calc->calc_kind == 'DATaxRulesBill') {
							...
							$calc_rules_vattax_amount += $calc->calc_amount;
						}
					}
				//}
			}
			...
			//for($i=0;$i<2;$i++){
				...
				if(VmConfig::get('debug_Sql')){
					vmdebug('Mysql udpateBill',$sql);
				}
			//}
	...
	function updateStatusForOneOrder($virtuemart_order_id, $inputOrder, $useTriggers=true){
		...
		if($upd_items) {
			...
			foreach($undhandled as $calc_kind){
				...
				$rule = null;
				//Find existing rule
				$sql = 'SELECT * FROM `#__virtuemart_order_calc_rules` WHERE virtuemart_order_id= "'.$virtuemart_order_id.'" AND `calc_kind` = "'.$calc_kind.'" ';
				$db->setQuery( $sql );
				//load result as object
				$ruleOld = $db->loadObject();

				$setTaxId = vRequest::getInt('order_'.$calc_kind.'_tax_id',0);
				vmdebug('ShipPay Rules handling $setTaxId',$setTaxId,$sql,$ruleOld);
				if($setTaxId > 0){
					//Todo, use table. A bit quickn dirty, Find existing rule
					$sql = 'SELECT * FROM `#__virtuemart_calcs` WHERE `virtuemart_calc_id` = "'.$setTaxId.'" ';
					$db->setQuery( $sql );
					$rule = $db->loadObject();

					if($rule){
						$rule->calc_rule_name = $rule->calc_rule_name;
						if($ruleOld) {
							$rule->virtuemart_order_calc_rule_id = $ruleOld->virtuemart_order_calc_rule_id;
						} else {
							//vmError('updateStatusForOneOrder No rule,$ruleOld found for '.$calc_kind.' '.$setTaxId);
						}
					}
				...
				if($rule === null) {
					...
					if($ruleOld){
						...
						$rule = $this->getTable('order_calc_rules')->loadFieldValues(false);
					}
					//$rule->calc_value = 0.0;
					//$rule->calc_amount = 0.0;

				}

				$rule->calc_kind = $calc_kind;
				$rule->virtuemart_order_id = $virtuemart_order_id;
				$rule->virtuemart_order_item_id = 0;
				$rule->virtuemart_calc_id = $setTaxId;

				vmdebug('UpdateStatusForOneOrder $rule for '.$calc_kind.' '.$virtuemart_order_id.' '.$setTaxId,$rule);
				...
				$t1 = 0.0;
				if($setTaxId > 0){
					$rule->calc_amount = $data->{$keyN} * ($rule->calc_value * 0.01 ) ;
					...
					vmdebug('Use rule by $setTaxId');
				} else {
					/*$sql = 'SELECT virtuemart_order_calc_rule_id FROM `#__virtuemart_order_calc_rules` WHERE virtuemart_order_id= "'.$virtuemart_order_id.'" AND `calc_kind` = "'.$calc_kind.'" ';
					$db->setQuery( $sql );
					$virtuemart_order_calc_rule_id = $db->loadResult();
					$rule->virtuemart_order_calc_rule_id = $virtuemart_order_calc_rule_id;*/

					if($rule->virtuemart_calc_id < 0){
						$rule->calc_amount = $data->{$keyNTax} = 0.0;
						$rule->calc_value = 0.0;
						$rule->calc_rule_name = 'None';

					} else {
						if($calc_kind == 'payment' or ($calc_kind == 'shipment' and VmConfig::get('radicalShipPaymentVat',true))){


							if ($rule->virtuemart_calc_id == 0) {
								/*$rule = $summarizedRules['taxBill'][$idWithMax];
								$rule->calc_kind = $calc_kind;
								$rule->virtuemart_order_id = $virtuemart_order_id;
								$rule->virtuemart_order_item_id = 0;
								$rule->virtuemart_order_calc_rule_id = $ruleOld->virtuemart_order_calc_rule_id;*/
								vmdebug('virtuemart_calc_id == 0  $summarizedRules ');
								$rule->calc_rule_name = $summarizedRules['taxBill'][$idWithMax]->calc_name;
								$t1 = 0.0;
								$rule->calc_amount = $data->{$keyNTax} = (float)$summarizedRules['taxBill'][$idWithMax]->calc_value * 0.01  * (float)$data->{$keyN};
							} else {
								$rule->calc_amount = $data->{$keyNTax} = 0.0;
							}
	...
	private function _createOrderCalcRules($order_id, $_cart)
	{
		...
		if(!empty($_cart->virtuemart_shipmentmethod_id)) {
			...
			if(empty($_cart->cartPrices['shipment_calc_id'])){
				vmdebug('create order calc shipment id empty');
				$orderCalcRules->calc_value = 0.0;
				$orderCalcRules->calc_rule_name = vmText::_('COM_VIRTUEMART_NONE');
			} else {
			...
			if (!$orderCalcRules->check()) {
				vmdebug('create order calc shipment check failed');
				return false;
			} else {
				vmdebug('create order calc shipment storing');
				// Save the record to the database
				if (!$orderCalcRules->store()) {
					vmdebug('create order calc shipment store failed');
					return false;
				}
			}

		} else {
			vmdebug('create order calc no shipment shipment_calc_id');
		}
