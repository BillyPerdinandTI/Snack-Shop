 ...
 * @version $Id: cart.php 11223 2025-11-05 21:31:16Z Milbo $
 */
...
//					unset($data->cartfields['tos']);
	...
	public function setCartIntoSession($storeDb = false, $forceWrite = false) {
		...
		if($this->storeCartSession and $storeDb){
			//vmdebug('setCartIntoSession execute storeCart');
			$this->storeCart();
		}

		if($forceWrite){
			//vmdebug('Force Writing of session');
			if (!headers_sent()) {
	...
	public function removeProductCart($pos) {
		...
		if(isset($this->cartProductsData[$pos])){
			...
			vmdebug('removeProductCart $pos '.$this->cartProductsData);
			return false;
		}
	}
	...
	public function updateProductCart() {
		...
		vRequest::vmCheckToken() or jexit ('Invalid Token updateProductCart');
		...
		foreach($quantities as $key=>$quantity){
			...
			if (isset($this->cartProductsData[$key]) and !empty($quantity) and !isset($_POST['delete_'.$key])) {
				//vmdebug('updateProductCart product update to',$key,$quantity,$_POST);
				...
				vmdebug('updateProductCart delete product from cart',$quantity,$_POST['delete_'.$key]);
				...
				vmdebug('updateProductCart product from cart ELSE nothing happens',$quantity);
			}
		}
	...
	public function setCouponCode($coupon_code) {
		...
		vRequest::vmCheckToken() or jexit ('Invalid Token setCouponCode');
		...
		if(empty($this->BT['email'])){
			VmInfo('Enter your email first before applying a coupon code.');
			return false;
		}

		$byUser = '';
		if($currentUser->id) {
			$byUser = ' OR vo.virtuemart_user_id = '.$currentUser->id.'  ';
		}

		$q = 'SELECT count(vo.virtuemart_order_id) FROM `#__virtuemart_order_userinfos` AS `oui` 
			INNER JOIN `#__virtuemart_orders` AS `vo` ON (vo.virtuemart_order_id = oui.virtuemart_order_id)
			WHERE (vo.coupon_code LIKE '.$db->quote($db->escape(trim($coupon_code))).' AND address_type="BT" 
			AND ( oui.email = "'.$this->BT['email'].'" '.$byUser.') )';
		...
		$query->join( 'INNER', $db->quoteName( '#__virtuemart_orderuserinfos', 'oui' ).' ON ('.$db->quoteName( 'vo.virtuemart_order_id' ).' = '.$db->quoteName( 'oui.virtuemart_order_id' ).')' );


		$query->where( $byUser.'vo.`coupon_code` LIKE \''.$db->escape(trim($coupon_code)).'\' AND address_type="BT" AND ( )' );*/
vmdebug('my coupoon query ',$q);
		$db->setQuery( $q );
	...
	public function redirecter($relUrl,$redirectMsg = ''){

		$this->_dataValidated = false;
		$app = JFactory::getApplication();
		//vmdebug('my redirecter '.$relUrl,(int)$this->_redirect,(int)$this->_redirected,(int)$this->_redirect_disabled);
		if($this->_redirect and !$this->_redirected and !$this->_redirect_disabled){
			...
			$this->_inCheckOut = false;
			$this->setCartIntoSession(false, false);
			//vmdebug('my redirecter, no redirect '.$relUrl);
			return false;
		}
	}
	...
	public function checkoutData($redirect = true, $forceStoreSession = false) {
		...
		if($this->_redirected){
			$this->_redirect = false;
			//vmdebug('checkoutData redirect false');
		} else {
			$this->_redirect = $redirect;
			//vmdebug('checkoutData not redirected, so set redirect to '.$this->_redirect);
		}
...
//			quorvia consider whether the store wants to populate empty ST address with the BT address
	...
	function confirmedOrder() {
		...
		if ($this->_dataValidated and $this->_confirmDone and !$this->_inCheckOut) {
			...
			$reUseTimeSql = VmConfig::get('reuseorders',0);
	...
	function saveAddressInCart($data, $type, $putIntoDb = true, $prefix='') {
		...
		foreach ($prepareUserFields as $fld) {
			if(!empty($fld->name)){
				...
				if(isset($data[$prefix.$name])){
					...
					$this->{$type}[$name] = $data[$prefix.$name] = $userFieldsModel->prepareFieldDataSave($fld, $data, $prefix);

					//We remove the default
					if(isset($this->{'byDefault'.$type}[$name])){
						unset($this->{'byDefault'.$type}[$name]);
					}
				} else {
		...
		unset($this->{$type}['password']);
		unset($this->{$type}['password2']);
