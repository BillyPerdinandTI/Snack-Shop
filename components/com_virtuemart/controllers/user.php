 ...
 * @version $Id: user.php 11217 2025-11-03 19:13:53Z Milbo $
 */
...
class VirtueMartControllerUser extends JControllerLegacy {
	...
	function saveUser(){
		...
		if($cart->_fromCart or $cart->getInCheckOut()){
			...
			if(!$msg){
				$this->setRedirect(JRoute::_('index.php?option=com_virtuemart&view=user&task=editaddresscart&addrtype=BT'.$task) );
			} else {
				if ($cart->getInCheckOut()){
					$task = '&task=checkout';

				}
				$this->setRedirect(JRoute::_('index.php?option=com_virtuemart&view=cart'.$task) );
			}
		} else {
			$layout = vRequest::getCmd('layout','edit');
			$this->setRedirect( JRoute::_('index.php?option=com_virtuemart&view=user&layout='.$layout) );
		}
	...
	public function saveData($cartObj) {
		...
		vRequest::vmCheckToken() or jexit ('Invalid Token saveData');
		...
		if($cartObj) unset($data['virtuemart_vendor_id']);
		...
		if($data['address_type'] == 'ST' and !$currentUser->guest){
			...
			if(/*$register and */$currentUser->guest and VmConfig::get('regNeedProdCart', true)){
				...
				if(empty($cart->cartProductsData)){
					...
					$app->redirect( JRoute::_($redirect_url) );
				}
			}
			...
			if($currentUser->guest==1 and ($register or !$cartObj )){
				...
				if (is_array($ret) and $ret['success'] and !$useractivation) {
					...
					$layout = vRequest::getCmd('layout','edit');
					$this->redirect( JRoute::_('index.php?option=com_virtuemart&view=user&layout='.$layout) );
				}
			}
		}
	...
	function removeAddressST(){
		...
		if($cart->_fromCart){
			...
			$this->setRedirect( JRoute::_('index.php?option=com_virtuemart&view=user&task=edit&virtuemart_user_id[]='.$virtuemart_user_id[0], null,$this->useSSL) );
		}
	...
	function checkCaptcha($retUrl){
			...
			if($msg !== TRUE){
				...
				//$errmsg = $e->getMessage();
				vmWarn($msg);
				$this->setRedirect (JRoute::_ ($retUrl . '&captcha=1') );
				return FALSE;
