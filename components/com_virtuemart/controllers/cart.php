...
class VirtueMartControllerCart extends JControllerLegacy {
	...
	public function display($cachable = false, $urlparams = false){
		...
		if(VmConfig::get('use_as_catalog', 0)){
			...
			$continue_link = JRoute::_('index.php?option=com_virtuemart&view=category' . $categoryLink . $ItemIdLink);
		...
		$view->cart = &$cart;
		shopFunctionsF::getRegisterGuestOptions($view);
		$cart->setupAddressFieldsForCart(true, false, $view->showRegistration);
		...
		if(($task == 'confirm' or isset($request['confirm'])) and !$cart->getInCheckOut()){
			...
			if(!$cart->_redirected){
				...
				vmdebug('cart display redirect ',(int)$redirect);
				if( !$redirect and VmConfig::get('directCheckout',false) and !$cart->getInCheckOut() and !vRequest::getInt('dynamic',0) and !$cart->_dataValidated) {
					$redirect = true;
			...
			vmdebug('Checkout without redirect, go to cart');
		}
	...
	public function checkout($html=true,$force = null){

		return $this->updatecart($html);
	}

	public function updatecart($html=true,$force = null, $methods = true) {

		//If we confirm an order, it should not be changed in anyway. So on one hand it would be right, to direct to display
		//On the other hand, the confirm is always meant for the current visible cart.
		...
		//if($currentUser->guest and !empty($data) /*and isset($_REQUEST['register'])*/ ){
		$saveUserData = vRequest::getInt('saveUserData', false);
		$register = isset($_REQUEST['register']);
		vmdebug('updatecart $saveUserData $register',$saveUserData,$register);
		if( $saveUserData  or $register ) {
			vRequest::vmCheckToken() or jexit ('Invalid Token saveAddressInCart');
			$data = vRequest::getPost();
			$cart->saveAddressInCart($data, 'BT',false);

			if ( ($currentUser->guest and $register) OR !$currentUser->guest ){
				vmdebug('updatecart save user data in cart',$currentUser);
		...
		if(!empty($coupon_code)){
			//vmdebug('my cartData["couponCode"]',$coupon_code, $cart->cartData['couponCode']);
	...
	public function addJS() {
		...
		if ($cart) {
			...
			$json->msg = '<a href="' . JRoute::_('index.php?option=com_virtuemart') . '" >' . vmText::_('COM_VIRTUEMART_CONTINUE_SHOPPING') . '</a>';
	...
	public function delete() {
		...
		$cart = VirtueMartCart::getCart();
		$cart->storeCartSession = true;
		$cartPos = vRequest::getInt('cartpos', 0);

		foreach($cartPos as $pos){
			if(isset($_POST['delete_'.$pos])){
				if ($cart->removeProductCart($pos))
