 ...
 * @version $Id: view.html.php 11224 2025-11-07 11:04:18Z Milbo $
 */
...
class VirtueMartViewCart extends VmView {
	...
	public function display($tpl = null) {
		...
		vmdebug('my layout cart display ',$this->_layout);
		...
		if ($this->layoutName == 'select_shipment') {
			...
			shopFunctionsF::getRegisterGuestOptions($this);
		...
		if(VmConfig::get('xTokOnCheckout',true)){
			$js = "if(jQuery('#checkoutForm').find('input[name=\"".vRequest::getFormToken()."\"]').length == 0){
				jQuery('#checkoutForm').append('<input type=\"hidden\" name=\"".vRequest::getFormToken()."\" value=\"1\" /> ');
			} ";
			vmJsApi::addJScript('tokencart', $js);
		}
		...
		if ($this->found_shipment_method == 0)  {
			...
			if ($validUserDataBT===-1) {
				if (VmConfig::get('oncheckout_opc', 1)) {
					...
					$this->cart->redirecter(JRoute::_('index.php?option=com_virtuemart&view=user&task=editaddresscart&addrtype=BT'));
				}
			}
	...
	Virtuemart.setUnCheckedRedBorder = function(){
        ...
        if(this.type == "checkbox"){
        	...
        	if(!this.checked){
				...
				$(this).siblings("label").addClass("form-control is-invalid");
				console.log("my setUnCheckedRedBorder required checkbox set red",this);
        	}
        } else if (this.type == "radio"){
...
jQuery(document).ready(function( $ ){
	...
	$(".output-shipto").find(":radio").bind("change", function(){
		...
		//if(!$cart->validateUserData()){
			$j = "
			Virtuemart.checkChanged = function (){
		
		var form = jQuery('.vm-checkout-register-userfields');
		var button = form.find('button[name=register]');
		if (typeof button == 'undefined') {
			alert('No button found');
			return;
		}
		
		var inputfields = form.find('input[type=text], input[type=password], select');
		//console.log('Found input fields:', inputfields);
		if (typeof inputfields !== 'undefined') {
			jQuery.each(inputfields, function () {
				jQuery(this).on(\"change\", function(){
					//alert('change');
					if(jQuery('#checkoutForm').find('input[name=\"saveUserData\"]').length > 0){
						return;
					} else {
						jQuery('#checkoutForm').append('<input type=\"hidden\" name=\"saveUserData\" value=\"1\" /> ');
					}
...
Virtuemart.checkChanged();
";
			vmJsApi::addJScript('checkChanged',$j);
		//}
