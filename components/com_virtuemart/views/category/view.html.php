 ...
 * @version $Id: view.html.php 11222 2025-11-05 20:02:55Z Milbo $
 */
...
class VirtuemartViewCategory extends VmView {
	...
	public function display($tpl = null) {
		...
		$this->categoryId = (int)vRequest::getInt('virtuemart_category_id', -1);
		if($this->categoryId === -1 and isset($menu->query['virtuemart_category_id'])){
			$this->categoryId = (int)$menu->query['virtuemart_category_id'];
		...
		if ((int)$this->categoryId === -2) {
			//stAn - i'd rather suggest own layout for 404 here instead of guessing last category, etc..
			return $this->handle404();
		...
		'omitLoaded_featured' => VmConfig::get('omitLoaded_featured',1),
		'featured_only_cat' => VmConfig::get('featured_only_cat',0),
		'discontinued' => VmConfig::get('discontinued',0),
		'discontinued_rows' => VmConfig::get('discontinued_rows',1),
		'omitLoaded_discontinued' => VmConfig::get('omitLoaded_discontinued',1),
		'discontinued_only_cat' => VmConfig::get('discontinued_only_cat',0),
		'latest' => VmConfig::get('latest',1),
		'latest_rows' => VmConfig::get('latest_rows',1),
		'omitLoaded_latest' => VmConfig::get('omitLoaded_latest',1),
		'latest_only_cat' => VmConfig::get('latest_only_cat',0),
		'topten' => VmConfig::get('topten',1),
		'topten_rows' => VmConfig::get('topten_rows',1),
		'omitLoaded_topten' => VmConfig::get('omitLoaded_topten',1),
		'topten_only_cat' => VmConfig::get('topten_only_cat',0),
		'recent' => VmConfig::get('recent',0),
		'recent_rows' => VmConfig::get('recent_rows',1),
		'omitLoaded_recent' => VmConfig::get('omitLoaded_recent',1),
		'recent_only_cat' => VmConfig::get('recent_only_cat',0)
		);
		...
		//vmdebug('$this->category category ',$this->category);
		foreach($paramNames as $k => $v){
			...
			if(isset($this->category->{$k}) and $this->category->{$k}!=''){
				$this->{$k} = $this->category->{$k};
				vmdebug('Use setting set by category '.$k,$this->{$k});
			} else {
				$this->{$k} = $menuParams->get($prefix.$k,$v);
				//vmdebug('Use setting set by menu '.$k,$this->{$k});
			}
		}
		...
		if ($dynamic and $id) {
			...
			if($search_enabled and !empty($this->keyword) or !empty($this->productModel->searchcustoms)) {
			...
			foreach( $opt as $o ) {
				...
				if($o == 'products') {
					...
					if(!empty( $this->{$o} ) and !empty( $this->{$o.'_rows'} )) {
						...
						if($this->{$o.'_only_cat'}  and $this->categoryId>0){
							$onlyCat = $this->categoryId;
						} else {
							$onlyCat = 0;

						}
						$this->products[$o] = $this->productModel->getProductListing( $o, $perRow * $this->{$o.'_rows'}, TRUE, TRUE, FALSE, $onlyCat, $onlyCat);
		...
		if ($this->products) {
			...
			foreach($this->products as $pType => $productSeries) {
				...
				foreach ($productSeries as $i => $productItem) {
					shopFunctionsF::triggerContentPlugin($productItem, 'product', 'product_name');
				}
...
//		quorvia get rid of any excess data
		...
		if($this->keyword !== false and !empty($this->keyword)){
			$title .=' ('.vRequest::vmSpecialChars(strip_tags(htmlspecialchars_decode($this->keyword))).')';
		}
	...
	public function handle404($cat = false){
		...
		if ((int)VmConfig::get('handle_404',1)) {
			...
			if((int)VmConfig::get('redirect_404',false)){
				...
				//stAn - so that we can adjust the template as needed
				$this->is404 = true;
				$this->display();
			}
		} else {
		...
		$app = JFactory::getApplication();
		//stAn - without this, J5 might ovewrite 404 into 200 !!
		if (method_exists($app, 'setHeader')) {
			$app->setHeader('Status', "404 Not Found", 'true'); 
			//important for frontend proxy cache:
