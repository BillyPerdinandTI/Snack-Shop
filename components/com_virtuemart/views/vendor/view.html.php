...
// 		if ($layoutName=='default') {
		if (empty($virtuemart_vendor_id)) {
			...
			$this->vendorFieldSkips = array('name','username','email','agreed');
			if(!empty($menu)){
				...
				$skips = !empty($menuParams->get('skips')) ? $menuParams->get('skips') : '';
				if(!empty($skips)){
					$skips = explode(';', trim($skips));
					$this->vendorFieldSkips = array_merge($this->vendorFieldSkips, $skips);
				}
			}
