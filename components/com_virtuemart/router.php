 ...
 * @subpackage router
 * @version $Id$
 * @copyright Copyright (C) 2009 - 2025 by the VirtueMart Team and authors
...
//if(version_compare(JVERSION,'4.0.0','ge')) {
	...
	/**
	 * Routing class derived from Joomla com_contact
	 *
	 * @since  3.3
	 */
	class VirtuemartRouter extends RouterView {
		...
		public function parse(&$segments) {
			...
			$ret = vmrouterHelper::virtuemartParseRoute($segments);
			if (!empty($ret)) {
				$segments = array();
			}
			return $ret;
		...
		public function preprocess($query) {
			vmRouterdebug('Execute preprocess',$query);
		...
		public function build(&$query) {
			...
			/*if(JVM_VERSION >3 and !vmrouterHelper::$full and !empty($query['view'])){
				if($query['view'] == 'category' or $query['view'] == 'productdetails'){
					unset($query['Itemid']);
				}
			}*/
...
function vmRouterdebug($debugdescr,$debugvalues=NULL){
	if(vmrouterHelper::$debug){
		VmEcho::$_debug = 1;
		if($debugvalues!==NULL) {
			$debugvalues = func_get_args();
			if (count($debugvalues) > 1) {
				array_shift($debugvalues);
			}
		}
...
class vmrouterHelper {
	...
	static protected $dbview = array(   'askquestion'   => 'askquestion',
										'cart'          => 'cart',
										'category'      => 'category',
										'invoice'       => 'invoice',
	...
	static function preprocess(&$query){
		...
		/*if(JVM_VERSION >3 and !vmrouterHelper::$full and !empty($query['view'])){
			...
			if($findItemid){
				...
				} else if(!empty(vmrouterHelper::$Itemid)){
		...
		switch ($view) {
			...
			case 'category';
				...
				if ( isset($query['virtuemart_category_id']) or isset($query['virtuemart_manufacturer_id']) ) {
					...
					//We dont need that, it is in the getCategoryRoute
					$categoryRoute = self::getCategoryRoute($catId, $manId);

					//http://forum.virtuemart.net/index.php?topic=121642.0
					//the self::$full is handled in the getCategoryRoute function, we use the found itemId only, if it is NOT the homepage
					if (!empty($categoryRoute->Itemid) and !empty($jmenu['virtuemart']) and $categoryRoute->Itemid!=$jmenu['virtuemart']) {
						vmRouterdebug('Router found itemid at category '.$categoryRoute->Itemid);
			...
			case 'productdetails';
				...
				//vmdebug('Product details segments',$query, $jmenu);
				$virtuemart_product_id = false;
				$ismenue = false;

				if (!empty($query['virtuemart_product_id']) and isset($jmenu['virtuemart_product_id']) ){

					if(isset($jmenu['virtuemart_product_id'][ $query['virtuemart_product_id'] ] ) ) {

						if(!empty($query['Itemid']) and $query['Itemid']!=$jmenu['virtuemart_product_id'][$query['virtuemart_product_id']] ){
						...
						//vmdebug('vmRouter case \'productdetails\' IS menu ',$query);
					} else if (isset($jmenu['virtuemart_product_id'][ 0 ])){
						$query['Itemid'] = $jmenu['virtuemart_product_id'][0];
						$ismenue = true;
						//vmdebug('vmRouter case \'productdetails\' IS menu ',$query);
					}
				}
				...
				if(!$ismenue) {
					//vmdebug('vmRouter case \'productdetails\' NOT menu ');
					...
					if(self::$full){
						...
						if(!(empty( $catId ) and empty( $manId ))){
							...
							$categoryRoute = self::getCategoryRoute($catId, $manId, $menuAlias);
							if ($categoryRoute->route) $segments[] = $categoryRoute->route;

							//vmTrace('link comes from',false,10);
							vmRouterdebug('vmRouter case \'productdetails\' no menu, full, not empty cat ',$query,$catId,$categoryRoute,$segments);
	...
	static function virtuemartParseRoute($segments) {
	...
	static function parseRoute($segments) {
		...
		if (  self::compareKey($segments[0] ,'manufacturer') ) {
			...
			if(!empty($segments[1])){
				...
				$vars['virtuemart_manufacturer_id'] = 0;
			}
...
// if joomla link joomlalink/vendorname/layout
		...
		if ( !empty(self::$seo_sufix_size) and ((substr($last_elem, -(int)self::$seo_sufix_size ) == self::$seo_sufix)
			...
			if(!self::$use_id) {
				...
				$vars['virtuemart_product_id'] = isset($product['virtuemart_product_id']) ? $product['virtuemart_product_id'] : 0;
				$vars['virtuemart_category_id'] = isset($product['virtuemart_category_id']) ? $product['virtuemart_category_id'] : 0;
	...
	static public function getLimitByCategory($catId, $view = 'category'){
		...
		if(empty($c[$catId][$view])){
			...
			if($view!='manufacturer'){	//Take care, this could be the categor view, just displaying manufacturer products
				...
				$cat = $catModel->getCategory($catId,0,0);
		...
		if (!isset(self::$_catRoute[$key])){
			...
			if (isset(self::$menu['virtuemart_category_id'])) {
				if (isset( self::$menu['virtuemart_category_id'][$catId][$manId])) {
					...
					//vmdebug('getCategoryRoute found menu for category '.$catId.' and manid '.$manId,$category);

				} else if(self::$full){
					$catModel->categoryRecursed = 0;
					$CatParentIds = $catModel->getCategoryRecurse($catId,0) ;
					//vmdebug('getCategoryRoute no menu but full '.$catId,$CatParentIds);
			...
			if ($ismenu==false) {
				...
				//vmdebug('getCategoryRoute $ismenu==false ',$menuCatid);
				//The getCategoryRoute is already doing that already
				...
				*/
				...
				//vmdebug('getCategoryRoute With $wMenuAlias, menu item found',$menuItem);
				if(!empty($menuItem)){
	...
	static public function getCategoryNames($catId,$catMenuId=0){
		...
		foreach ($parent_ids as $id ) {
			if(!isset($categoryNamesCache[self::$slang][$id])){
				...
				$cat = $catModel->getCategory($id,0,0);
...
//vmRouterdebug('getProductName '.self::$slang, $prTable->_langTag,VmConfig::$vmlang,$prTable->slug);
		...
		if(!isset($prodIds[$hash])){
			...
			if(!empty($categoryName)){
				...
				if($product){
					...
					$prodIds[$hash]['virtuemart_category_id'] = $product->virtuemart_category_id;
				} else {
					vmdebug('Router, product not found ',$names);
				}
	...
	static public function setMenuItemId(){
		...
		if(empty($menuVmitems)){
			...
			//vmRouterdebug('my menuVmItems',$menuVmitems, self::$menu);
				...
				if(!empty($link['view'])){
					...
					if($dbKey){
						if($dbKey=='category'){
							...
							vmdebug('setMenuItemId found a cart menu item',$link);
			...
			//@Todo I wonder if this still makes sense
			if(self::$byMenu){
				foreach ($menuVmitems as $item)	{
	...
	static public function findCorrectItemid(&$query){
		...
		if ($view == 'category' or $view == 'productdetails') {
			...
			//vmdebug('Find Itemid for cart view',$query);
			$layout = (empty( $query['layout'] )) ? 0 : $query['layout'];
